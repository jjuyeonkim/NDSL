...
from ndsl.utils import load_f90nml, f90nml_as_dict, DEFAULT_GRID_NML_GROUPS
...
class NmlParser(ConfigParser):
    ...
    def parse(self, filepath: Path) -> dict[str, Any]:
        ...
        for config_name, target_groups in config_nml_map.items():
            standardized_data[config_name] = f90nml_as_dict(nml, flatten=True, target_groups=target_groups)
    ...
    def parse(self, filepath: Path) -> dict[str, Any]:
        ...
        for config_name, yaml_key in config_yaml_map.items():
            if yaml_key in data.keys():
                standardized_data[config_name] = data[yaml_key] # TODO do I need to do a deep copy?
            else:
                standardized_data[config_name] = {}
        return standardized_data
